...
#define  BSP_LED_START_BIT  (3 - 1)                            /* LEDs[3:1] are sequentially connected to PTC[5:3].  */
...
void  BSP_Init (void)
{
    ...
    RCC_PLLConfig(RCC_PLLSource_HSE_Div1, RCC_PLLMul_9);      /* PLL = Fprediv1 * 9 = 72Mhz.                         */
    RCC_PLLCmd(ENABLE);
    ...
    BSP_CPU_ClkFreq_MHz = BSP_CPU_ClkFreq_MHz;                  /* Surpress compiler warning BSP_CPU_ClkFreq_MHz    ... */
	 ...
	 /* Init Pins for Logic Analysis */
	  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
    gpio_init.GPIO_Pin   = LOGIC_DBG_PIN1;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
    gpio_init.GPIO_Mode  = GPIO_Mode_Out_PP;
    GPIO_Init(LOGIC_DBG_PORT1, &gpio_init);

	  //RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
    gpio_init.GPIO_Pin   = LOGIC_DBG_PIN2;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
    gpio_init.GPIO_Mode  = GPIO_Mode_Out_PP;
    GPIO_Init(LOGIC_DBG_PORT2, &gpio_init);
...
static  void  BSP_LED_Init (void)
{
    ...
    gpio_init.GPIO_Pin   = BSP_GPIOB_LED;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
    gpio_init.GPIO_Mode  = GPIO_Mode_Out_PP;
...
{
  GPIO_SetBits(GPIOB, BSP_GPIOB_LED);
}
...
{
   GPIO_ResetBits(GPIOB, BSP_GPIOB_LED);
}
...
void  BSP_LED_Toggle (void)
{
  ...
  pins ^= BSP_GPIOB_LED;
  GPIO_SetBits(  GPIOB,   pins  & BSP_GPIOB_LED);
  GPIO_ResetBits(GPIOB, (~pins) & BSP_GPIOB_LED);
}
